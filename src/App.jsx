...
const GlobalStyles = () => (
  <style>{`
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    body {
      background: #0d0d10; color: #e8e8f0;
      font-family: 'DM Sans', system-ui, sans-serif;
      -webkit-font-smoothing: antialiased; overflow-x: hidden; width: 100%; min-width: 0;
    }
    #root { display: flex; flex-direction: column; min-height: 100vh; overflow-x: hidden; width: 100%; }
    ...
    input[type=range]::-webkit-slider-thumb { appearance: none; width: 12px; height: 12px; background: #E60023; border-radius: 50%; }
    ...
    @media (min-width: 640px) { .features-grid { grid-template-columns: repeat(4, 1fr); gap: 12px; } }
    ...
    @media (min-width: 480px) { .toast { left: auto; right: 24px; bottom: 24px; max-width: none; margin: 0; } }
    ...
    /* Extra bottom padding so content isn't hidden behind AnchorAd */
    ...
    <div className="features-grid" style={{ marginBottom: '48px' }}>
      {features.map((f, i) => (
        <div key={i} style={{
          ...
          borderRadius: '16px', display: 'flex', flexDirection: 'column', gap: '6px',
...
export default function App() {
  ...
  useEffect(() => { setPremiumState(isPremium); }, [isPremium, setPremiumState]);
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    setStatus('loading'); setMedia(null); setErrorMsg('');
    try {
      ...
      setMedia(data.media); setStatus('success');
  ...
  const handlePaste = async () => {
    try {
      ...
      if (text.includes('pinterest') || text.includes('pin.it')) {
        setUrl(text); showToast('URL pasted!');
      }
    } catch {}
  };

  const handleDownload = useCallback((mediaType, action) => {
    gateDownload(mediaType, () => { action(); showToast('Download started!'); });
  ...
  const inputBorderColor = isFocused
    ? 'rgba(230,0,35,0.4)'
    : status === 'error' ? 'rgba(230,0,35,0.3)' : 'rgba(255,255,255,0.09)';
    ...
    <>
      ...
      {toast && (
        <div className="toast" style={{
          ...
          borderRadius: '12px', color: toast.type === 'error' ? '#ff7090' : '#4ade80',
          ...
          <div className="container">
            <div style={{ maxWidth: '720px', margin: '0 auto', textAlign: 'center' }}>
              ...
              <h1 style={{ fontSize: 'clamp(28px, 8vw, 66px)', fontWeight: 800, fontFamily: 'Syne, sans-serif', color: '#ededf0', lineHeight: 1.08, letterSpacing: '-0.03em', marginBottom: 'clamp(12px,3vw,20px)', animation: 'fadeUp 0.55s ease 0.1s both' }}>
                Save Pinterest<br /><span className="gradient-text">Videos Instantly</span>
              </h1>
              <p style={{ fontSize: 'clamp(14px, 3.5vw, 18px)', color: '#888897', fontFamily: 'DM Sans, sans-serif', lineHeight: 1.7, marginBottom: 'clamp(24px,5vw,40px)', animation: 'fadeUp 0.55s ease 0.15s both', padding: '0 4px' }}>
                Preview any Pinterest video before downloading it in full original quality. No watermarks. No registration. Just MP4.
              </p>

              {/* Input card */}
              <div className="hero-input-card" style={{
                background: '#121215', border: `1px solid ${inputBorderColor}`, borderRadius: '20px',
                marginBottom: '16px',
                boxShadow: isFocused ? '0 0 0 4px rgba(230,0,35,0.07), 0 20px 60px rgba(0,0,0,0.4)' : '0 20px 60px rgba(0,0,0,0.35)',
                transition: 'border-color 0.25s, box-shadow 0.25s', animation: 'fadeUp 0.6s ease 0.2s both',
              }}>
                <div className="input-row">
                  <div style={{ color: '#E60023', opacity: 0.6, display: 'flex' }}><PinIcon size={16} /></div>
                  <input
                ...
                <div className="action-buttons">
                  <button className="paste-btn" onClick={handlePaste} style={{ padding: '11px 14px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px', color: '#888897', fontSize: '13px', fontWeight: 500, fontFamily: 'DM Sans, sans-serif', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>
                    ⊕ Paste
                  </button>
                  <button onClick={handleAnalyze} disabled={status === 'loading'} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '12px 20px', background: status === 'loading' ? 'rgba(230,0,35,0.5)' : 'linear-gradient(135deg, #E60023 0%, #b8001c 100%)', border: 'none', borderRadius: '10px', color: '#fff', fontSize: 'clamp(13px,3.5vw,15px)', fontWeight: 700, fontFamily: 'Syne, sans-serif', cursor: status === 'loading' ? 'not-allowed' : 'pointer', boxShadow: status === 'loading' ? 'none' : '0 4px 20px rgba(230,0,35,0.35)', transition: 'all 0.2s', whiteSpace: 'nowrap' }}>
                    {status === 'loading' ? (<><span style={{ display: 'inline-block', width: '15px', height: '15px', border: '2px solid rgba(255,255,255,0.3)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin 0.7s linear infinite', flexShrink: 0 }} /> Analyzing…</>) : 'Analyze & Preview'}
                  </button>
                </div>
